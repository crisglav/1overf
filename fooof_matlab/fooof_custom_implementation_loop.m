...
for iFile =1:length(subset)
    ...
    self.r2_full = corr(self.pow_orig', self.fooofed_spectrum_full')^2;
    self.mae_full = mean(abs(self.pow_orig - self.fooofed_spectrum_full));
    self.mse_full = mean((self.pow_orig - self.fooofed_spectrum_full).^2);
    self.rmse_full = sqrt(mean((self.pow_orig - self.fooofed_spectrum_full).^2));
