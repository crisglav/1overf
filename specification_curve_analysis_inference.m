...
addpath('/rechenmagd3/Experiments/2023_1overf/results/features/vdata');
...
% Open log file
fid = fopen(fullfile(results_path,'logfile_rand.txt'),'a');
...
% Define at which randomization to start based on the ones that have runned
% already
frand = dir(fullfile(results_path),'specs_ap_exp_rand*');
startRand = 
...
for iRand=1:500
    ...
    save(fullfile(results_path,sprintf('specs_ap_exp_rand%.3d.mat',iRand)),'exp','r_squared','mae');
end
t002 = toc(t001);
fprintf(fid,'Script ended successfulli. It took: %.2f hours \n',t002/(60*60));
close(fid);
