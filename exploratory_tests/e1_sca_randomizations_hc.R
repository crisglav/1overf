...
outpath <- file.path(scapath,"randomizations_e1_hc")
...
# Healthy mask
hc_mask = as.logical(mat_data$hc.mask)
apexp_hc = mat_data$exp[,hc_mask]
age_original = participants$age[hc_mask]

# Number of specifications, and participants
nSpec = dim(apexp_hc)[1]
nSubj =  dim(apexp_hc)[2]
