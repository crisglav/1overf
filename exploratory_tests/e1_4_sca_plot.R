...
filename = "C:\\Users\\Cristina\\1overf\\results\\sca\\randomizations_e1\\stats_orig.csv"
stats = read.csv(filename, header = TRUE, sep = ",")
results <- cbind(specifications,stats)

# load data from inference
filename <- "C:\\Users\\Cristina\\1overf\\results\\sca\\e1_sca_inference.csv"
...
R_highlighted = results$R[spec_highlighted]
...
########### plot curve   ###########################
p1 <-  results %>%
  ggplot(aes(x = specifications, y = R, ymin = R, ymax = R)) +
  geom_hline(yintercept= 0, linetype = "solid", color = "black", linewidth = 0.3) +
  geom_point(aes(color = log10(BF)), size = 0.75) +
  geom_point(aes(x = spec_highlighted, y = R_highlighted, color = log10(bf_highlighted)), size = 4, shape = 124) +
  scale_color_viridis(limits = c(-1, 1),option = "viridis", breaks = c(-1, log10(1/3), 0, log10(3),1), oob = scales::squish) + # Set out-of-bounds color to nearest limit
  theme_minimal() +
...
p2 <- results %>%
  ...
  scale_color_viridis(limits = c(-1, 1), option = "viridis", oob = scales::squish) +
  theme_minimal() + 
...
p3 <- ggplot() +
  ...
  theme_minimal() + 
  theme(panel.grid.minor = element_blank(), panel.grid.major = element_line(linewidth = 0.25)) +
  labs(x = "specifications (nr), sorted by effect size", y = "R") +
  ylim(-0.25, 0.25)

########### join plots and save as pdf #####################
pdf("C:\\Users\\Cristina\\1overf\\results\\sca\\e1_sca.pdf")
