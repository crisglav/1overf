...
apexp_cwp_hc = cwp.res_apexp_h1(cwp_hc_mask);
apexp_cwp_pa = cwp.res_apexp_h1(cwp_pa_mask);

cbp_hc_mask = cbp.group == 'hc';
cbp_pa_mask = cbp.group == 'pa';
apexp_cbp_hc = cbp.res_apexp_h1(cbp_hc_mask);
apexp_cbp_pa = cbp.res_apexp_h1(cbp_pa_mask);

% for hypothesis 2
apexp_cwp_h2 = cwp.res_apexp_h2(cwp_pa_mask);
pain_cwp_h2 = cwp.res_pain_h2(cwp_pa_mask);
apexp_cbp_h2 = cbp.res_apexp_h2(cbp_pa_mask);
pain_cbp_h2 = cbp.res_pain_h2(cbp_pa_mask);
...
ylabel('Aperiodic exponents (age-corrected)');
box off
grid on
...
% tlc = tiledlayout(1,2);
ax=nexttile;
model_cbp = fitlm(pain_cbp_h2, apexp_cbp_h2);
h = plot(model_cbp);
h(1).Marker = 'o';
...
h(4).LineWidth = 1.5;
xlabel('Pain intensity (age-corrected)');
ylabel('Aperiodic exponents (age-corrected)');
title('');
box off;
legend off;
...
model_cwp = fitlm(pain_cwp_h2, apexp_cwp_h2);
nexttile;
h = plot(model_cwp);
h(1).Marker = 'o';
