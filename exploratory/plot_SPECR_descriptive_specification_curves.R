...
filename = "C:\\Users\\Mitarbeiter\\1overf\\results\\sca\\specs_bf.txt"
results = read.csv(filename, header = TRUE, sep = ",")


########## prepare data ############################

# sort results, add specification number and define colors according to BF10
results <- results %>% 
  dplyr::arrange((.data$effect_size))  
results <- results %>% 
  dplyr::mutate(specifications = 1:n(), 
                color = case_when(bayes_factor > 3 ~ "#377eb8", 
                                  bayes_factor < .3333 ~ "#e41a1c",
                                  TRUE ~ "darkgrey"))
...
highlightedData <- data.frame(effect_size, indexhighlightedSpecs, confLow, confHigh)
...
########### plot curve   ###########################
p1 <-  results %>%
  ggplot(aes(x = .data$specifications, y = .data$effect_size, 
             ymin = .data$effect_size, ymax = .data$effect_size, color = .data$color)) +
  ...
  geom_point(data = highlightedData, 
             aes(x = .data$indexhighlightedSpecs, y = .data$effect_size),
...
pdf("results_1overf_specr_curve.pdf")
